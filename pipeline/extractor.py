...
# 모델 변경 이력: gemini-flash-latest → gemini-2.5-flash → gemini-2.5-flash-lite → gemini-flash-lite-latest → gemini-3.1-flash-lite → gemini-3.1-flash-lite-preview
...
[액션아이템 추출 기준]
아래 표현이 포함된 발화만 추출하세요:
- 확정 약속: "~할게요", "~하겠습니다", "~맡겠습니다"
- 지시·요청: "~해주세요", "~부탁드려요", "~푸쉬할게요"
- 이미 수행 중인 행동: "~요청했는데", "~보냈는데" (발화자가 담당자)
- 재확인 확정: "~그대로", "~그대로 가는 거", "~변동 없음"
  → 이미 합의된 사항을 재확인하는 발화. 단, "그대로"가 포함되더라도 단순 상황 설명이면 추출하지 않음

아래 표현은 액션아이템이 아니므로 추출하지 마세요:
...
- "오전에 요청했는데", "내가 보냈는데" → 이미 수행 중, 발화자가 담당자
...
[few-shot 예시 1 - 확정 약속 / 회의 날짜: 2026-06-01 월요일]
...
[few-shot 예시 3 - 이미 수행 중인 행동 → 발화자가 담당자]
발화: "오전 열 시쯤 담당자에게 요청했는데 아직 답이 없어요" (수아/퍼포먼스 마케터)
출력:
{"action": "광고주 담당자에게 누끼 컷 전달 요청", "assignee": "수아", "deadline": null, "source_utterance": "오전 열 시쯤 담당자에게 요청했는데 아직 답이 없어요"}

[few-shot 예시 4 - 담당자 불명확]
...
발화: "네, 채널별 성과랑 예산 배분 좀 보고, 아 그리고 타겟 세그먼트 누락된 거 있어서 그것도 같이 봐야 할 것 같아요" (수아/퍼포먼스 마케터)
출력:
[]

[few-shot 예시 6 - 재확인 확정 / 회의 날짜: 2026-06-01 월요일]
발화: "제안서는 어쨌든 목요일 오후 그대로 가는 거니까" (지훈/마케팅 팀장)
출력:
{"action": "제안서 제출", "assignee": null, "deadline": "2026-06-04 오후 (목요일 오후)", "source_utterance": "제안서는 어쨌든 목요일 오후 그대로 가는 거니까"}"""

# 회의 날짜·요일 + 이전 청크 추출 결과를 user 메시지에 포함
# {previous_actions_section}: 첫 청크면 빈 문자열, 이후 청크면 중복 방지 목록
...
def _build_previous_actions_section(previous_actions: list) -> str:
    """이전 청크 추출 결과를 중복 방지 안내 문구로 변환."""
    if not previous_actions:
        return ""
    lines = []
    for a in previous_actions:
        assignee = a.get("assignee") or "미정"
        lines.append(f"- {a['action']} (담당자: {assignee})")
    return "\n이미 추출된 액션아이템 (아래 항목과 동일하거나 중복되면 추출하지 마세요):\n" \
           + "\n".join(lines) + "\n"


def _call_gemini_api(chunk_text: str, meeting_date: str,
                     previous_actions: list = None, retry: bool = False) -> str:
    ...
    user_content = _USER_PROMPT_TEMPLATE.format(
        ...
        previous_actions_section=_build_previous_actions_section(previous_actions or []),
...
def _call_llm_with_retry(chunk_segments: list, meeting_id: str, meeting_date: str,
                         previous_actions: list = None, max_retries: int = 3) -> list:
...
def extract_actions(segments: list, meeting_id: str, meeting_date: str) -> list:
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        # 직전까지 추출된 액션 목록을 전달 → 중복 추출 방지
        actions = _call_llm_with_retry(
            chunk, meeting_id, meeting_date,
