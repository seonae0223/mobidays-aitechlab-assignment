...
```
 [mp3 입력]                      [transcript JSON 입력]
회의 녹음 파일                   검증용 transcript 파일
      ↓                                  ↓
pipeline/stt.py                          ↓
Whisper 로컬 STT                         ↓
(음성 → 텍스트 변환)                      ↓
      ...
      └──────────────┬───────────────────┘
           ...
           pipeline/cleaner.py
           텍스트 정제 + 청크 분리
           (머뭇거림 제거 / 약어 정규화 / 흐릿한 표현 플래그)
                     ↓
           pipeline/extractor.py
           Gemini API 액션아이템 추출
           (few-shot / 날짜 추론 / confidence 계산)
                     ↓
           pipeline/loader.py
           DuckDB 적재 (멱등성 보장)
                     ↓
...
대시보드 위젯 4개가 전부 집계 쿼리입니다. SQLite는 행 기반이라 집계가 느리고, Postgres는 서버가 필요해 로컬 재현성이 낮습니다. DuckDB는 파일 하나로 관리되며 컬럼형 집계에 최적화되어 있어 선택했습니다.

### Gemini API (Google) (LLM)

무료 티어 제공, 한국어 성능, 구조화 출력(JSON) 지원이 이유입니다. 모델: `gemini-flash-latest`.
...
LLM 자체 confidence 대신 룰 기반으로 계산합니다. 블랙박스 없이 설명 가능하고 수정 가능합니다.
